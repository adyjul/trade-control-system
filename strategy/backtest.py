...
def trend_rider_signal(row, df, ma_fast=50, ma_slow=100):
    ...
    trend = 'UP' if ma50 > ma100 else 'DOWN' if ma50 < ma100 else 'NEUTRAL'

    if row['macd'] > row['macd_signal'] and row['rsi'] > 50 and trend == 'UP':
        return 'LONG'
    elif row['macd'] < row['macd_signal'] and row['rsi'] < 50 and trend == 'DOWN':
        return 'SHORT'
    else:
        return 'HOLD'
    
def apply_trend_rider(df):
    """
    Terapkan trend rider ke dataframe hasil backtest
    """
    df['trend_rider_signal'] = df.apply(lambda row: trend_rider_signal(row, df), axis=1)

    # Bisa kombinasikan filter false reversal
    df['trend_rider_signal'] = df.apply(
        lambda row: 'HOLD' if is_false_reversal(row, df) else row['trend_rider_signal'],
        axis=1
    )
    return df
